...
## [2.4.1] - 2025-01-12 (進捗管理改善版)

### 🎯 新機能
- **進捗リセットボタン**: 設定パネルに「進捗リセット」ボタンを追加
  - `.progress.json`ファイルを手動で削除可能
  - 削除前に保存データ件数を表示する確認ダイアログ
  - 誤操作防止のための警告表示
  - ワンクリックで進捗をリセットして再処理が可能

### 🔧 改善
- **処理完了メッセージの透明性向上**: スキップ件数を明示的に表示
  - 例: `✓ 8件処理完了（2件スキップ）`
  - ユーザーが実際に何件処理されたか一目で分かる
  - スキップがない場合は従来通りの表示

### 🐛 修正
- **進捗再開機能の挙動を改善**: ユーザーの混乱を解消
  - 進捗ファイルが存在する場合、前回処理済みのASINが自動スキップされる仕様を明確化
  - スキップされたASINの件数を処理完了時に表示することで透明性を向上

### 📝 ユースケース
この更新は以下の状況で特に有用です：
- 大量処理の途中で意図的に中断し、後で続きから再開したい場合
- テスト用に同じASINを何度も処理したい場合
- 進捗ファイルが残っていて、最初から処理し直したい場合

---

## [2.4.0] - 2025-01-10 (大規模スクレイピング堅牢化版)

### 🚀 新機能
- **RateLimiterクラス**: インテリジェントなレート制限管理
  - 通常時: 4-9秒のランダム待機
  - CAPTCHA検出時: 30秒ペナルティ + 待機時間2倍
  - 429エラー時: 待機時間1.5倍
  - 成功時: 徐々に回復（0.5倍ずつ）
- **自動リトライ機能**: 一時的なエラーを自動回復
  - 最大5回の自動リトライ
  - 指数バックオフ（1.2倍）
  - 対象エラー: 429, 500, 502, 503, 504
- **User-Agentローテーション**: 12種類のリアルなUser-Agentからランダム選択
...
### ⚙️ 設定管理
- **config.json設定**: すべてのパラメータをカスタマイズ可能
  - `min_delay`: 最小待機時間（デフォルト4.0秒）
  - `max_delay`: 最大待機時間（デフォルト9.0秒）
  - `penalty_delay`: CAPTCHA時の追加待機（デフォルト30.0秒）
  - `batch_size`: バッチサイズ（デフォルト25）
  - `batch_cooldown`: バッチ間待機（デフォルト60秒）
  - `max_retries`: 最大リトライ回数（デフォルト5）
  - `backoff_factor`: リトライ時の待機倍率（デフォルト1.2）

### 🖥️ GUI改善
- **新しいprocess_asins()統合**: キーワード抽出モードで新機能を全て使用
  - バッチ処理とクールダウン
  - 進捗保存と再開
  - リアルタイム進捗コールバック
  - 停止判定コールバック
- **ブランド名取得モード**: 従来の処理を維持（10-15秒待機）

### 📊 ログ改善
- **詳細ログ出力**: すべての処理ステップを可視化
  - `[START]` 処理開始
  - `[BATCH]` バッチ処理
  - `[WAIT]` レート制限待機
  - `[OK]` 成功メッセージ
  ...
  - `[COMPLETE]` 処理完了
  - `[STATS]` 最終統計
- **Unicode安全性**: Windows環境でのエンコードエラーを回避
  - try-except でUnicodeEncodeErrorを捕捉
  - 絵文字を`[OK]`, `[WARNING]`などに置換

### 🔧 技術改善
- **セッション管理**: `requests.Session()`でクッキー・コネクション再利用
- **エラーハンドリング強化**: メトリクス追跡と自動リカバリー
- **依存関係更新**: `urllib3>=1.26.0`, `requests>=2.28.0`を追加
...
### ✅ 実測結果
- **145件連続取得成功**: 失敗率0%で動作確認済み（2025-01-10テスト実施）
  - 全てのASINで商品情報取得に成功
  - CAPTCHA検出なし
  - 自動リトライ機能が正常動作
  - バッチ処理（6バッチ）が安定稼働
...
### ⚠️ 注意事項
- キーワード抽出モードでは新しいバッチ処理を使用
- ブランド名取得モードは従来の処理（10-15秒待機）を維持
- 初回実行時は少数件でテスト推奨
- `.progress.json`は自動生成される進捗ファイル
